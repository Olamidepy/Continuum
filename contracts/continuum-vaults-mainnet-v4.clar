;; Continuum Vaults Contract (Clarity 4 Mainnet Version)
...
(define-data-var sbtc-contract-address principal 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token)
    ...
    (let
      ...
      (let
        ...
        (if (is-eq (get asset-type vault) "STX")
          ...
          (begin
            (asserts! (is-eq (contract-of sbtc-token) (var-get sbtc-contract-address)) ERR-INVALID-TOKEN)
            (try! (contract-call? sbtc-token transfer additional-amount tx-sender current-contract none))
  ...
  (let
    ...
    (let
      ...
      (if (is-eq (get asset-type vault) "STX")
        (begin
          (try! (try! (as-contract? 
            ((with-stx (total-claimable)))
            (stx-transfer? total-claimable tx-sender (get owner vault))
          )))
          (ok total-claimable)
        )
        (begin
          (asserts! (is-eq (contract-of sbtc-token) (var-get sbtc-contract-address)) ERR-INVALID-TOKEN)
          (try! (try! (as-contract?
            ((with-ft sbtc-token total-claimable tx-sender))
            (contract-call? sbtc-token transfer total-claimable tx-sender (get owner vault) none)
          )))
          (ok total-claimable)
        )
      )
      ...
      (let
        ...
        (if (is-eq (get asset-type updated-vault) "STX")
          (begin
            ...
            (try! (try! (as-contract?
              ((with-stx (payout)))
              (stx-transfer? payout tx-sender (get owner updated-vault))
            )))
          )
          (begin
            (asserts! (is-eq (contract-of sbtc-token) (var-get sbtc-contract-address)) ERR-INVALID-TOKEN)
            ...
            (try! (try! (as-contract?
              ((with-ft sbtc-token payout tx-sender))
              (contract-call? sbtc-token transfer payout tx-sender (get owner updated-vault) none)
            )))
          )
        )
        (ok payout)
          ...
          (let
            ...
            (if (> remaining-shares u0)
              (let
                ...
                (try! (try! (as-contract?
                  ((with-stx (treasury-part)))
                  (stx-transfer? treasury-part tx-sender (var-get treasury-address))
                )))
              )
              (try! (try! (as-contract?
                ((with-stx (penalty)))
                (stx-transfer? penalty tx-sender (var-get treasury-address))
              )))
            )
          )
          (let
            ...
            (if (> remaining-shares u0)
              (let
                ...
                (try! (try! (as-contract?
                  ((with-ft sbtc-token treasury-part tx-sender))
                  (contract-call? sbtc-token transfer treasury-part tx-sender (var-get treasury-address) none)
                )))
              )
              (try! (try! (as-contract?
                ((with-ft sbtc-token penalty tx-sender))
                (contract-call? sbtc-token transfer penalty tx-sender (var-get treasury-address) none)
              )))
            )
          )
        )
